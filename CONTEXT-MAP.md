...
- [pi](./docs/pi/CONTEXT.md) - pi 扩展业务域：发现嵌套 skill 与以 `$` 插入并展开 skill，以及经 DeepSeek 的 web 搜索与直连 web 抓取。
...
- **pi → Nano Flow**：pi 插件使 Nano Flow 以嵌套 skill 形式分发的主流程 skill 能被发现，并支持以 `$` 引用在任意位置插入与展开。

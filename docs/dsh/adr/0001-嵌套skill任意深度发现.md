...
DSH 内建 provider 只发现 `.agents/skills/<skill>/SKILL.md` 的一层形态，Nano Flow 等技能把主流程 skill 嵌套在 `references/skills/` 下因而不可见。本域以独立 DSH 插件在项目与用户 `.agents` 树中任意深度发现携带合法 frontmatter 的 `SKILL.md`，注册名取 frontmatter `name` 原样，并让嵌套 candidate 的 rank（250）高于内建一层形态（200），使顶层显式声明在同名师冲突时胜出。

...
3. **`stateOf` 参数顺序反了（既有缺陷）**：`host.ts` 调 `live.stateOf("subagent", session)`，`dsh-session-projection` 的签名是 `stateOf(session, key)`（alpha.2 与 rc.1 相同）；实现按 `key` 查注册表，传入 Session 对象时命中不到，静默返回 `undefined`，子会话读取因此总走冷路径。
...
- **修正 `stateOf` 调用**，恢复 live 投影快速路径；子会话模式仍解析 `values["subagent"].identity.mode`。
...
- **`stateOf` 契约**：`SessionProjectionsLike.stateOf(session, key)`，与 `dsh-session-projection` 签名一致。
...
- 修复 `inject` 缺陷（改用 `ctx.get` 组装服务）与 `stateOf` 参数顺序缺陷，并补可拦截同类回归的用例。
...
| #   | Issue                                               | 状态        | 阻塞于 | 下一步         |
| --- | --------------------------------------------------- | ----------- | ------ | -------------- |
| 01  | [依赖声明对齐](01-依赖声明对齐.md)                  | in_progress | —      | /code-delivery |
| 02  | [inject 与 stateOf 修复](02-inject与stateOf修复.md) | pending     | —      | /code-delivery |

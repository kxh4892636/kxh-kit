...
无未决产品选项。实际发布包帮助已在 Node.js 24.19.0 成功执行；共享 DSH_HOME 的真实多实例与版本切换验证由 Issue 01/02 承担，未宣称兼容性已验证。若验证事实导致设计实质改变，返回 dev-gate，不降低验收标准。
...
| #   | Issue                                          | 状态        | 阻塞于 | 下一步         |
| --- | ---------------------------------------------- | ----------- | ------ | -------------- |
| 01  | [后台启动重启与保活](01-后台启动重启与保活.md) | in_progress | —      | /code-delivery |
| 02  | [自动更新与失败回退](02-自动更新与失败回退.md) | pending     | 01     | /code-delivery |

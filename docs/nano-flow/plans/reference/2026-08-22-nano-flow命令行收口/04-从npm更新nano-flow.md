...
使用者显式执行 `nf self update` 时，可检测并安装 npm 中的 CLI 版本，然后将已安装受管 skill 同步到同版；失败时保留原可用状态。
...
- `nf self update` 默认 selector 为 `latest` 且不选 prerelease；`--version <semver-or-tag>` 覆盖 selector。
...
验证证据：31 个 Nano Flow 测试通过；包构建通过；全仓 `vp check` 为 0 error（31 个既有 warning）；本机 Node 直启 npm CLI 返回 11.6.2；Standards/Spec 双轴审查均无 blocker。

...
一个可本地打包、安装和运行的 `@kxh4892636/nano-flow` 包，以 deep CLI kernel 统一内建子命令的 options、help、JSON、错误和预演契约。

## 范围

- 建立 `packages/nano-flow/package.json`、TypeScript 配置、`src/main.ts`、bin 构建与包文件清单，保留已有 skill 源文件。
...
- [x] 从本地 `.tgz` 安装到临时 npm prefix 后，`nf --help`、fixture 各级 `--help` 和 fixture 预演均给出约定结果。
...
- 交付物：`packages/nano-flow/package.json`、`packages/nano-flow/src/cli/`、build-time builtin discovery、fixture builtin 与可执行 `dist/main.mjs`。
- 验证证据：`vp run @kxh4892636/nano-flow#test`，13 个 interface 测试通过。
- 静态门禁：`vp run @kxh4892636/nano-flow#build` 与 `vp check` 通过；全仓仅保留既有 warning。

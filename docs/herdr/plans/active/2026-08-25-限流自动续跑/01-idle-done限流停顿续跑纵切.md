...
- 实现全 session agent list、本地 `idle`/`done` 状态过滤、规范化 `recent_unwrapped` 快照末尾 233 Unicode code point 匹配。
...
- [x] 匹配测试覆盖 Unicode 空白规范化、按 code point 截取恰好 233 字符、`limit` 大小写不敏感、token 落在窗口外不命中。
...
- 交付物: Herdr socket 端口、手工 `scan-now` action、233 Unicode code point 检测、发送前身份复核、跨进程成功指纹和脱敏诊断。

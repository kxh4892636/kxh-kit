...
- [ ] 插件使用 `agent.read` 的 `recent_unwrapped` 快照，先把换行及连续空白规范化为单个空格并去除首尾空白，再把末尾 233 个 Unicode 字符定义为“当前最新消息区域”。
...
- Herdr 没有语义消息边界，“当前最新消息”固定近似为规范化 `recent_unwrapped` 快照末尾 233 个 Unicode 字符。

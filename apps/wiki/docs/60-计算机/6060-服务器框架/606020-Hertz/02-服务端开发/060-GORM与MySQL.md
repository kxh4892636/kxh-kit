...
- 实践: 所有事务调用使用同一个带 context 的 DB handle;
- 实践: 不在事务内执行不受控的远程 HTTP 调用;
- 实践: 唯一键冲突映射为 409 领域冲突，不把 MySQL 错误文本返回客户端;
...
- 官方入口: [GORM 错误处理](https://gorm.io/docs/error_handling.html)、[GORM 更新](https://gorm.io/docs/update.html);

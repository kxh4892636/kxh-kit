...
- 超时: 默认值与跨进程传播前提见 [超时与重试](../02-服务端开发/020-超时与重试.md);
- 协议: 通过 [协议互通表](../90-协议速查/020-传输与序列化协议.md) 对齐 IDL、Client 与 Server;
- Middleware: 先注册者包裹后注册者；公共配置必须说明注入顺序;
- 限流: 内置与自定义同类 limiter 的覆盖规则见 [流量保护](../02-服务端开发/030-熔断降级与流量保护.md);
- 连接: 旧 mux 的废弃状态与连接池边界见 [消息与连接](../90-协议速查/010-消息类型与连接.md);

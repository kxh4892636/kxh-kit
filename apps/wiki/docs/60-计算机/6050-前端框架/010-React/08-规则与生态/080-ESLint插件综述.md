...
## eslint-plugin-react-hooks 的作用是什么?

- 在构建期强制执行 Rules of React;
- 捕获 Hooks 调用错误、不纯渲染、遗漏依赖等问题;

## eslint-plugin-react-hooks 如何安装?

```bash
npm install -D eslint eslint-plugin-react-hooks
```

## eslint-plugin-react-hooks 的推荐配置是什么?
...
## eslint-plugin-react-hooks 覆盖哪些规则?
...
## ESLint 插件的价值是什么? 与 StrictMode 如何互补?

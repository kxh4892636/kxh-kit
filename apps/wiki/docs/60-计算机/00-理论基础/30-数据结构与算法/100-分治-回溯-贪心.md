...
var findMedianSortedArrays = function (nums1, nums2) {
  ...
  const n = left;
  const m = k - left;
  const v1 = Math.max(n <= 0 ? -Infinity : nums1[n - 1], m <= 0 ? -Infinity : nums2[m - 1]);
  ...
  const backtrack = (row, col, visited, index, word, board) => {
    ...
    if (word[index] === board[row][col]) {
      ...
      for (const [dx, dy] of dirctionList) {
        const curCol = col + dx;
        const curRow = row + dy;
        if (curCol < 0 || curCol >= cols || curRow < 0 || curRow >= rows || visited[curRow][curCol])
          continue;
...
var removeKdigits = function (num, k) {
  const minStack = [];
  for (const ch of num) {
    while (k > 0 && minStack.length !== 0 && ch < minStack[minStack.length - 1]) {
      minStack.pop();
      k--;
    }

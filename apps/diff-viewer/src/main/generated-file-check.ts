...
const matchesAnySuffix = (fileName: string, suffixes: readonly string[]): boolean =>
...
const isWordString = (text: string): boolean => {
  if (text.length === 0) {
    return false;
  }
...
const hasGeneratedSuffix = (fileName: string, marker: ".generated." | ".gen."): boolean => {
...
const LANGUAGE_SPECIFIC: LanguagePattern[] = [
  {
    extensions: [".go"],
    patterns: [
      // Go 官方约定 (https://golang.org/s/generatedcode)
...
// 三层判定: 1. 路径/文件名 (不读文件); 2. 通用文件头; 3. 语言专属文件头。
// 文件内容经 getHeaderLines 惰性提供 (最多前 20 行), 避免每个文件都付出读取代价
export const isGeneratedFile = (
  ...
  // markdown 多为文档且常引用生成头示例, 跳过内容检查以免误报

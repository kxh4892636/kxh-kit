...
const createTarget = async (): Promise<string> => {
  const target = await mkdtemp(path.join(tmpdir(), "loopx-self-mutation-"));
...
describe("self mutation boundaries", (): void => {
  test("preserves the update failure as the cause after a successful rollback", async (): Promise<void> => {
    ...
    await expect(plan.commit()).rejects.toMatchObject({
      message: "LoopX update failed and was rolled back to 1.0.0",
      cause: installError,
    });
  ...
  test("reports both update and rollback failures", async (): Promise<void> => {
    ...
    await expect(plan.commit()).rejects.toStrictEqual(
      new AggregateError(
        ...
        "LoopX update and rollback both failed; requested restoration to 1.0.0",
      ),
    );
  });
});
...
const createScript = (overrides: Partial<NpmScript> = {}): NpmScript => ({
  ...
  packOutput: JSON.stringify([{ filename: "nested/loopx.tgz" }]),
  skills: {
    "loop-x-cli": { "SKILL.md": "cli" },
    "loop-x": { "SKILL.md": "flow", "references/FLOW.md": "protocol" },
  },
  ...overrides,
});
...
describe("npm package manager adapter", (): void => {
  ...
  test("installs and rolls back through the same global npm contract", async (): Promise<void> => {
    ...
    expect(script.calls).toEqual([
      { executable: "npm", arguments_: ["install", "--global", "@kxh4892636/loopx@2.0.0"] },
      { executable: "npm", arguments_: ["install", "--global", "@kxh4892636/loopx@1.0.0"] },
    ]);
  });

  test.each([
    [JSON.stringify({ version: "1.0.0" }), "invalid LoopX version response"],
    [JSON.stringify(["invalid"]), "invalid LoopX version response"],
    [JSON.stringify(["2.0.0-beta.1"]), "No stable LoopX version matches latest"],
  ])(
    ...
    async (versionOutput: string, cause: string): Promise<void> => {
      ...
      await expect(manager.resolve("latest", false)).rejects.toMatchObject({
        message: "Unable to resolve @kxh4892636/loopx@latest",
  ...
  test.each([
    ["view", "resolve", "Unable to resolve @kxh4892636/loopx@latest"],
    ["pack", "resolve", "Unable to resolve @kxh4892636/loopx@latest"],
    ["tar", "resolve", "Unable to resolve @kxh4892636/loopx@latest"],
    ["install", "install", "Unable to install @kxh4892636/loopx@2.0.0"],
    ["install", "rollback", "Unable to restore @kxh4892636/loopx@2.0.0"],
  ] as const)(

...
export interface ResolvedLoopxPackage {
  readonly version: string;
  readonly skills: readonly ManagedSkill[];
}

export interface PackageManagerPort {
  resolve(selector: string, includePrerelease: boolean): Promise<ResolvedLoopxPackage>;
...
export const prepareSelfUpdate = async (
  ...
  const preview: JsonValue = {
    ...
    cli: { action: "install", package: "@kxh4892636/loopx", version: candidate.version },
      ...
      try {
        ...
        try {
          ...
          throw new AggregateError(
            [error, rollbackError],
            `LoopX update and rollback both failed; requested restoration to ${currentVersion}`,
          );
        }
        throw new Error(`LoopX update failed and was rolled back to ${currentVersion}`, {
          cause: error,
        });
      }
    },

...
跨业务域工作覆盖所有相关业务域，并读取 `CONTEXT-MAP.md` 中的关系。任一文件不存在时静默继续；术语或决策得到确认后，由 `/grill-with-docs` 按需创建或维护。

## 布局

领域内容只创建在工作区根目录的 multi-context 布局中；本约束文件随 `/loop-x` 分发：
...
- 所需概念尚未收录时，先判断它是否只是项目未使用的语言；确认存在领域缺口后，由 `/grill-with-docs` 维护 glossary。

...
const parseDependencies = (rawValue, targetPath, errors, rootDir) => {
	...
	try {
		...
		if (
			!Array.isArray(value) ||
			value.some((dependency) => !/^\d{2}$/.test(dependency))
		) {
...
const hasSection = (content, heading) =>
	new RegExp(`^## ${heading}$`, "m").test(content);
		...
		for (const dependency of issue.dependencies) {
			...
			if (!dependencyIssue) {
				addError(
					...
					rootDir,
					issue.path,
					`blocked_by 引用了不存在的 Issue ${dependency}`,
				);
				continue;
			}
			if (Number(dependency) >= Number(issue.id)) {
				addError(
					...
					rootDir,
					issue.path,
					`依赖 ${dependency} 必须排在 Issue ${issue.id} 之前`,
				);
			}
			if (
...
const checkIssue = (issuePath, id, errors, rootDir) => {
	...
	if (!ISSUE_STATUSES.has(status)) {
		addError(
			...
			rootDir,
			issuePath,
			`status 必须是 ${[...ISSUE_STATUSES].join(" | ")}`,
		);
	}
	const dependencies = parseDependencies(
		frontmatter.get("blocked_by"),
		issuePath,
		errors,
		rootDir,
	);
	...
	if (
		status === "blocked" &&
		(!content.includes("障碍") || !content.includes("解除条件"))
	) {
		addError(errors, rootDir, issuePath, "blocked Issue 必须记录障碍和解除条件");
	}
	if (
		status === "completed" &&
		(!/^## (交付记录|交付物与证据)$/m.test(content) ||
			!content.includes("证据"))
	) {
		addError(
			errors,
...
const checkPlanLayout = (planPath, lifecycle, errors, rootDir) => {
	...
	if (
		!planNameMatch ||
		!isValidDate(planNameMatch[1]) ||
		!HAN_PATTERN.test(planNameMatch[2])
	) {
		addError(
			errors,
			rootDir,
	...
	for (const entry of markdownFiles) {
		const allowed =
			["story.md", "spec.md"].includes(entry.name) ||
			/^\d{2}-.+\.md$/.test(entry.name);
		if (!allowed) {
			addError(
				errors,
				rootDir,
	...
	for (const issue of issues) {
		...
		if (row.fileName !== issue.fileName) {
			addError(
				...
				rootDir,
				specPath,
				`Issue ${issue.id} 链接应指向 ${issue.fileName}`,
			);
		}
		...
		if (row.dependencies.join(",") !== issue.dependencies.join(",")) {
			addError(
				...
				rootDir,
				specPath,
				`Issue ${issue.id} 表格依赖与 blocked_by 不一致`,
			);
		}
	}
...
const checkSpec = (planPath, lifecycle, markdownFiles, errors, rootDir) => {
	...
	const specFrontmatter = parseFrontmatter(
		specContent,
		specPath,
		errors,
		rootDir,
	...
	if (specStatus !== expectedSpecStatus) {
		addError(
			...
			rootDir,
			specPath,
			`聚合状态应为 ${expectedSpecStatus}，实际为 ${specStatus}`,
		);
	}
...
const checkPlans = (domainPath, errors, rootDir) => {
	...
	for (const lifecycleEntry of lifecycleEntries) {
		...
		for (const planEntry of readEntries(
			lifecyclePath,
			errors,
			rootDir,
		).filter((entry) => entry.isDirectory())) {
			checkPlan(
				path.join(lifecyclePath, planEntry.name),
				lifecycleEntry.name,
				errors,
				rootDir,
			);
...
const checkAdrs = (domainPath, errors, rootDir) => {
	...
	for (const [index, adrFile] of adrFiles.entries()) {
		...
		if (
			!nameMatch ||
			nameMatch[1] !== expectedId ||
			!HAN_PATTERN.test(nameMatch[2])
		) {
			addError(
				errors,
				rootDir,
...
export const checkDomain = (rootDirectory) => {
	...
	const mapContent = fs.existsSync(mapPath)
		? readText(mapPath, errors, rootDir)
		: null;
	...
	for (const domainEntry of domainEntries) {
			...
			if (content !== null) {
				...
				if (lineCount > 610) {
					addError(
						...
						rootDir,
						contextPath,
						`CONTEXT.md 共 ${lineCount} 行，超过 610`,
					);
				}
			}
		}
		if (
			mapContent !== null &&
			!mapContent.includes(`(./docs/${domainEntry.name}/CONTEXT.md)`)
		) {
	...
	if (mapContent !== null) {
		for (const match of mapContent.matchAll(
			/\(\.\/docs\/([^/]+)\/CONTEXT\.md\)/g,
		)) {
...
const run = () => {
	...
	try {
		...
		console.error(
			`领域文档校验异常: ${error instanceof Error ? error.stack : String(error)}`,
		);
		process.exitCode = 1;
	}
};

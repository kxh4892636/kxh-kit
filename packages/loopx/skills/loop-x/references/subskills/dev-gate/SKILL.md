---
name: dev-gate
description: 在进入 /implement 前, 使用 /grilling, /grill-with-docs 或 /to-issues 之后触发, 确认工作环境, 单一执行契约和供 /code-test 与 /verifying 使用的质量门禁等基线, 进行执行前的准入门禁检查.
---
...
用户依次确认 `工作环境`, `执行契约` 和 `质量门禁` 三项基线信息;

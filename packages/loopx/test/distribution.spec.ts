...
const executeBin = async (
  ...
  if (program === undefined) throw new Error("Unable to resolve installed LoopX bin");
...
const parseObject = (stdout: string): Record<string, unknown> => {
  ...
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new Error("Installed LoopX returned non-object JSON");
  }
...
const createDistributionFixture = async (): Promise<DistributionFixture> => {
  const sandbox = await mkdtemp(path.join(tmpdir(), "loopx-distribution-"));
  try {
    ...
    return {
      bin:
        process.platform === "win32"
          ? path.join(prefix, "loopx.cmd")
          : path.join(prefix, "bin", "loopx"),
...
const verifySkillLifecycle = async (fixture: DistributionFixture): Promise<void> => {
  ...
  for (const name of ["loop-x", "loop-x-cli"]) {
...
describe("installed LoopX distribution", (): void => {

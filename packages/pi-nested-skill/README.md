...
In the interactive editor, typing `/` at the beginning of input uses Pi's native
command completion; typing `/` after other text opens the loaded skill completion
list for inline markers. In the interactive transcript, inline skill blocks are
summarized as visible `[skill] <name>` markers so later expanded skills do not
disappear inside Markdown HTML rendering.

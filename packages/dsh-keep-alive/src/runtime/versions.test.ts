...
test("已安装版本复用，拒绝目录穿越和损坏元数据", async (): Promise<void> => {
  ...
  expect(await installLatest(f.paths.versions, launch, npm)).toEqual(f.version);
...
test("精确版本在安装完成后才发布，安装错误可重试", async (): Promise<void> => {
  ...
  await expect(installLatest(root, launch, npm)).rejects.toThrow(/network/);
  ...
  expect((await installLatest(root, launch, npm)).version).toBe("2.0.0-rc.1");
});

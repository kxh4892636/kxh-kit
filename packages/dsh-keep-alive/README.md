...
首次启动从配置的 npm registry 安装目标版本的 `@deepseek-ai/dsh`，解析为精确版本后保存；已缓存的可运行版本可离线启动。配置的源缺少该包或版本时（npm 报 `ETARGET`/`E404`），会自动改用 npm 官方源 `https://registry.npmjs.org` 重试一次——镜像源同步滞后时不必手工改配置；网络故障不会触发该回退，避免把等待翻倍。首次查询及安装总限时 10 分钟；启动就绪限时 60 秒。就绪要求受管进程存活、端口属于其进程树且 HTTP 可达。

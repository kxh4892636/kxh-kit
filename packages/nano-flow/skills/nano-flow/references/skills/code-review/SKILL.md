...
| Agent         | 来源与输出                                                                                                                                  |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **Standards** | 读取全部 standards 与 smell baseline；按文件、hunk 报告规则违反，另标 baseline smell，区分硬规则与 judgement；引用代码与规则，不超过 377 字 |
| **Spec**      | 读取全部 spec；报告缺失/部分实现、scope creep、行为存在但实现不符的要求；引用代码与 spec，不超过 377 字。Spec 不可用时不派遣                |

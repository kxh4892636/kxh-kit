...
完整读取 `<nano-flow-skill-root-dir>/references/DOMAIN.md` 与 `<nano-flow-skill-root-dir>/references/FLOW.md`，以其中的领域布局、Plan 生命周期和状态转移为权威约束。
...
4. 每次暂停前同步 issue 状态、spec 派生视图与证据，并按 `<nano-flow-skill-root-dir>/references/FLOW.md` 续租或释放；恢复时只信任文件与运行态，不依赖先前对话。

...
const createTarget = async (): Promise<string> => {
  const target = await mkdtemp(path.join(tmpdir(), "nf-skill-state-"));
...
describe("self skill query interface", (): void => {
  test("treats missing, invalid, and malformed managed markers as unmanaged", async (): Promise<void> => {
    ...
    for (const marker of [
      ...
      await writeFile(path.join(directory, ".nano-flow-managed.json"), marker, "utf8");
      await expect(readManagedMarker(directory)).resolves.toBeNull();
    }
  });
  ...
  test("wraps marker and skill tree filesystem failures", async (): Promise<void> => {
    ...
    await mkdir(path.join(directory, ".nano-flow-managed.json"), { recursive: true });
  ...
  test("lists packaged skills without writing the target", async (): Promise<void> => {
    ...
    expect(result).toEqual({
      skills: ["nano-flow", "nano-flow-cli"].map(
  ...
  test("installs, checks, and uninstalls nano-flow-cli", async (): Promise<void> => {
    const target = await createTarget();
    await invoke(target, ["install", "--name", "nano-flow-cli"]);
    expect(await invoke(target, ["check", "--name", "nano-flow-cli"])).toMatchObject({
      name: "nano-flow-cli",
      status: "current",
    });
    await invoke(target, ["uninstall", "--name", "nano-flow-cli"]);
    expect(await invoke(target, ["check", "--name", "nano-flow-cli"])).toMatchObject({
      name: "nano-flow-cli",
      status: "not_installed",
    });
...
describe("self skill query interface", (): void => {
  test("installs and uninstalls every packaged skill", async (): Promise<void> => {
    ...
    expect(await invoke(target, ["list"])).toMatchObject({
      skills: [
        { name: "nano-flow", status: "current" },
        { name: "nano-flow-cli", status: "current" },
      ],
    });

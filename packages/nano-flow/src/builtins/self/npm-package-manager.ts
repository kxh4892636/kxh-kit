...
const parseVersions = (stdout: string): readonly string[] => {
  ...
  throw new Error("npm returned an invalid Nano Flow version response");
};
...
const retrieveSkills = async (
  ...
  const temporaryRoot = await mkdtemp(path.join(tmpdir(), "nf-package-"));
  try {
    ...
    resolve: async (
      ...
      try {
        ...
        if (version === undefined)
          throw new Error(`No stable Nano Flow version matches ${selector}`);

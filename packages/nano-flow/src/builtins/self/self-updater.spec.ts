...
import type { PackageManagerPort, ResolvedNanoFlowPackage } from "./self-updater";
...
const managedSkill = (version: string, content: string): ManagedSkill => {
  ...
  return { name: "nano-flow", version, contentHash: hashSkillFiles(files), files };
};

const scriptedPort = (
  state: ScriptState,
  candidate: ResolvedNanoFlowPackage,
): PackageManagerPort => ({
  resolve: async (
    selector: string,
    includePrerelease: boolean,
  ): Promise<ResolvedNanoFlowPackage> => {
...
const createTarget = async (): Promise<string> => {
  const target = await mkdtemp(path.join(tmpdir(), "nf-self-update-"));
...
describe("self update orchestration", (): void => {
  test("updates latest and synchronizes installed skills", async (): Promise<void> => {
          ...
          changes: [
            {
              ...
              fromVersion: "1.0.0",
              name: "nano-flow",
              source: "package://skills/nano-flow",
              target: path.join(target, "nano-flow"),
              toVersion: "2.0.0",
            },
          ],
        },
    ...
    expect(await readFile(path.join(target, "nano-flow", "SKILL.md"), "utf8")).toBe("new");
  });
  ...
  test("dry-run returns the CLI and skill plan without changing either", async (): Promise<void> => {
    ...
    await installCurrentSkill(current, target, dependencies);
    const before = await readFile(path.join(target, "nano-flow", "SKILL.md"), "utf8");
    ...
    expect(await readFile(path.join(target, "nano-flow", "SKILL.md"), "utf8")).toBe(before);
  });
...
describe("self update orchestration", (): void => {
  ...
  test("rolls back CLI after install or skill synchronization failure", async (): Promise<void> => {
    ...
    expect(await readFile(path.join(target, "nano-flow", "SKILL.md"), "utf8")).toBe("old");

  ...
  try {
    ...
    if (note.audio !== undefined) {
      ...
      for (const audioItem of note.audio) {
        await validateMediaUrl(audioItem.url, urlConfig);
    ...
    if (note.picture !== undefined) {
      ...
      for (const pictureItem of note.picture) {
        await validateMediaUrl(pictureItem.url, urlConfig);
        pictureItem.filename = sanitizeMediaFilename(pictureItem.filename);
      }
    }

    const notesInfo = await client.invoke<Array<Record<string, unknown>>>("notesInfo", {
      notes: [note.id],
    });
    ...
    if (note.audio !== undefined) {
      (updateParams["note"] as Record<string, unknown>)["audio"] = note.audio;
    }
    if (note.picture !== undefined) {
      (updateParams["note"] as Record<string, unknown>)["picture"] = note.picture;
    }

    await client.invoke<null>("updateNoteFields", updateParams);

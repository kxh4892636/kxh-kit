import { z } from "zod";
import { ReadOnlyModeError } from "../../client/errors";
...
export const runAddNotes = async (
  ...
  try {
    ...
    // 1. 校验模板(一次调用)
    const fieldNames = await client.invoke<string[] | null>("modelFieldNames", {
      modelName,
    });
    ...
    // 3. 逐条调用 addNote(客户端内部串行化), 部分成功语义
    ...
    for (let i = 0; i < notes.length; i++) {
      ...
      try {
        const noteId = await client.invoke<number | null>("addNote", {
          note: noteParams,
        });

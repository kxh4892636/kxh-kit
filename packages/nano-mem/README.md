...
Within each matched-word tier, relevance is normalized as `(-BM25) / max(-BM25)` within that tier. Ranking uses an additive score: `0.618 * relevance + 0.382 * (0.618 * retrievability + 0.382 * useScore)`. Successful-use score grows logarithmically and caps at 100 uses. Equal total scores are ordered by original BM25 relevance, then memory ID.

On an initialized database with the current schema, search is read-only: it does not increment retrieval counts, change memory state, or acquire a write transaction. Existing retrieval-count fields remain for database compatibility and do not affect ranking. First-use initialization and necessary schema migrations may still write to the database.

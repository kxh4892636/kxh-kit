...
1. 根据用户目标和当前修改范围确定商家类型、目标 app 与目标页面。完成标准：商家类型、`shopId` 来源、目标 URL 来源都有明确记录。
   1. 测试商家、测试 `shopId`、fake login、抖店测试商家登录，读取 [test-merchant-login.md](references/test-merchant-login.md)。
   2. 线上真实商家、商家附身、附身抖店，读取 [production-merchant-impersonation.md](references/production-merchant-impersonation.md)。

...
本 reference 用于守护 BITS 项目流水线，确认目标 PPE/BOE 部署完成，或定位部署前失败原因。流水线验收仍遵循“检查 → 修复 → 最小重验”：检查目标 run，修复失败原因，只重验新的目标 run 和受影响的浏览器路径。
...
- 已确认守护范围：develop 任务、目标端型、目标项目流水线、目标环境。
- 已确认守护 run 与目标 commit/branch 对齐；不能用旧 run 结论替代本次 push。
- 需要部署的每条项目流水线都已完成 PPE/BOE 部署，或已定位失败节点和责任类型。
- 失败已进入代码修复、平台安全重试、待确认或阻塞；修复后只守护新目标 run 和受影响验收路径；没有为了制造新 push 做无意义改动。
...
多个 pipeline ID 全部纳入守护范围。主流水线用于定位项目流水线和整体阻塞；项目流水线用于判断实际部署完成。
...
部署完成后继续进入商家态和浏览器 E2E 验收，不等待后置人工确认节点。

## 多流水线规则

- develop 任务包含多个项目时，所有需要目标环境部署的项目流水线都必须部署完成。
...
1. 所有目标项目流水线部署完成，进入商家态和浏览器 E2E 验收。
2. 部署或其之前节点失败，且判断为代码问题，进入修复代码、重新 push、重新守护新流水线；部署完成后只重验受影响的浏览器路径。
...
- 代码问题：lint、build、test、类型检查、SCM 编译、产物构建等由代码导致的失败。修复代码后重新 push，并守护新流水线；新流水线部署完成后进入最小重验。
...
排查 BITS lint/build/SCM 失败时，本地验证必须使用仓库声明的 pnpm：
...
运行前读取根 `package.json.packageManager`。如果 `pnpm -v` 与仓库声明不一致，先校正工具链版本；不要把 lockfile config mismatch、依赖状态检查失败等工具链问题误报为代码失败。

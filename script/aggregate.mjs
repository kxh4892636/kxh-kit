...
function main() {
  ...
  for (const name of skillDirs) {
    ...
    if (!WHITELIST.includes(name)) {
      ...
      const body = stripFrontmatter(fs.readFileSync(path.join(dest, "SKILL.md"), "utf8"));
    ...
    rows.push(`| [${escapeCell(skillName)}](${relPath}) | ${escapeCell(description)} |`);
    console.log(`已聚合: ${name}${WHITELIST.includes(name) ? " (白名单, 原封不动)" : ""}`);
  }

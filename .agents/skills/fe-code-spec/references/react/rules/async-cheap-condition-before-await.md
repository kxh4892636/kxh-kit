...
This matters when `getFlag` hits the network, a feature-flag service, or another expensive async source: skipping it when `someCondition` is false removes that cost on the cold path.

...
6. 生成最终 Markdown 时，所有 `摘要:` 内容必须由模型翻译或改写为中文。GitHub Trending 章节需要仿照 weekly 条目的呈现方式，为每个仓库写中文摘要，并保留仓库链接、语言、Stars、Forks、本周新增 Stars 等脚本提供的事实字段。标题、链接、来源名、作者/机构名、仓库名可以保留原文；不要留下整句英文摘要。
...
- 如果同一文章出现在多个 weekly 中，必须合并为一个条目，保留所有来源、issue、栏目和发布日期；不要在最终文档里重复输出同一条内容。
- GitHub Trending 默认读取 weekly 页面中的周热门仓库，脚本抓 README 摘录作为模型生成中文摘要的依据。

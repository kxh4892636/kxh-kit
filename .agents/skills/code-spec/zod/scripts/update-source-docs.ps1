param(
  [string]$DownloadUrl = "https://codeload.github.com/colinhacks/zod/zip/refs/heads/main",
  [string]$ZipPrefix = "zod-main/packages/docs/content/"
)
...
$commitSha = $null
try {
  $commitOutput = git ls-remote "https://github.com/colinhacks/zod" refs/heads/main 2>$null
...
$snapshot = [ordered]@{
  source_url = "https://github.com/colinhacks/zod/tree/main/packages/docs/content"
  download_url = $DownloadUrl
  branch = "main"
  source_subdirectory = "packages/docs/content"
  ...
  notes = @(
    ...
    "The complete docs content directory is mirrored under references/source-docs."
  )
}

$snapshot | ConvertTo-Json -Depth 5 | Set-Content -Path (Join-Path $referencesRoot "snapshot.json") -Encoding utf8
Write-Host "Updated Zod source docs: $fileCount files"

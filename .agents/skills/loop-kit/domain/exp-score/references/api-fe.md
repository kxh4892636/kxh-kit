# Experience Score API FE

体验分前端创建或更新接口的 SOP。处理 H5/PC V10 请求、体验分 Kit API hook、BAM 生成包、非 BAM 手写请求、端应用接口消费前，应用本文件。

## 目标边界

新增体验分 V10 请求优先沉淀到 `@govern-public/experience-score` Kit，由 H5/PC 端应用通过 Kit 导出的稳定 hook 发起。

H5/PC 端应用需要请求时，从 Kit 根入口消费 hook，不直接调用 `@govern-public/api-governance-shop-api` 的生成函数发起网络请求。端应用可以通过 `import type` 使用 BAM 生成的 TypeScript 类型；具体 hook 名称、参数和返回类型必须读取当前代码后确定。
...
3. **封装 Kit API hook**
   ...
   - 完成标准：业务组件通过 hook 发请求，不感知 BAM 运行时请求函数、平台差异或 request 细节；所需 BAM 类型使用 type-only import。
...
5. **导出并消费**
   ...
   - H5/PC V10 端应用从 `@govern-public/experience-score` 消费 hook；需要显式类型时，可以从 BAM 生成包 type-only import。
   - 完成标准：端应用没有新增本地 service，也没有直接调用 BAM 生成函数发起网络请求。
...
- 端应用构建缺依赖：请求 hook 应从 `@govern-public/experience-score` 导入；BAM 类型从 `@govern-public/api-governance-shop-api` 使用 type-only import，避免引入运行时请求代码。

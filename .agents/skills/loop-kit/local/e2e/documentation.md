...
3. 单一可部署单元受影响时，使用该单元下的 `test/e2e/`。
4. 多个可部署单元共同组成一条路径时，使用共同项目根下的 `test/e2e/`，并在文档中列出参与单元。

`<e2e-dir>/index.md` 维护长期稳定回归流程；单次需求使用 `<e2e-dir>/yyyy-mm-dd-xxx.md`，日期取创建当天本地日期，`xxx` 使用简短 kebab-case 名称。项目已有不同约定时沿用项目约定。
...
`<e2e-dir>/index.md` 只承载已由真实执行证明、可长期复验的路径。

- 合入前需求文档中的场景状态必须为 passed，执行记录包含环境、版本、时间、关键断言和证据。
- 保留稳定场景 ID、入口、前置数据、动作、等待条件、断言和证据要求，使执行者只读 `index.md` 即可复现。
- draft / failed / blocked / 待确认和一次性排障步骤留在需求文档。
- 新流程替换旧流程时同步删除或改写过期步骤。完成标准：同一能力不存在互相冲突的入口、前置条件或验收口径。

---
name: code-spec
description: 代码规范;编写,修改或审查代码时使用.
---
...
- 单个文件不超过 610 行, 单个函数不超过 89 行, 超过时重构, 优先按职责和复用边界进行重构;
...
- 外部边界数据默认不可信, 使用项目 schema/validator 收窄类型.

## 测试原则

- 根据项目上下文, 使用单元测试/组件测试/E2E测试;

## TS

- 函数使用箭头函数, 不使用普通函数, 函数参数和返回值必须有类型注解..
- 禁止使用 `enum`, 使用 `const = {} as const` 替代, 需要值联合类型时, 从 const 对象派生.
- 外部边界数据默认不可信, 递归使用 `?` 可选链, 避免空指针异常.

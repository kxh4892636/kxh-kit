...
Use when a remote skill is stored as a child folder under another local skill and its common rules are also embedded directly in the parent skill, such as `vite-plus`, `shadcn`, `hono`, and `drizzle-orm` inside `code-spec`.

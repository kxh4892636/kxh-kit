...
| `drizzle-orm` | `https://github.com/drizzle-team/drizzle-orm-docs/tree/main/src/content/docs` | reference 模块：`.agents/skills/code-spec/references/drizzle-orm`；父级 guidance：`.agents/skills/code-spec/SKILL.md` backend 相关章节 | 将本地 reference 模块复制到临时目录，在该临时副本中运行 `scripts/update-source-docs.ps1` 以刷新 docs 快照，然后在同步有意义变更前，比较刷新后的临时模块与 `.agents/skills/code-spec/references/drizzle-orm`。保留本地 Drizzle 安全 guidance、evals 和 `code-spec` 父级路由入口。如果上游主题或文件名发生变化，更新 `references/doc-map.md`。 |
...
| `hono` | `https://github.com/honojs/website/tree/main/docs` | reference 模块：`.agents/skills/code-spec/references/hono`；父级 guidance：`.agents/skills/code-spec/SKILL.md` backend 相关章节 | 使用 sparse checkout 克隆 `https://github.com/honojs/website` 的 `docs`，比较获取到的 docs 与 `.agents/skills/code-spec/references/hono/references/source-docs`，然后只同步有意义的 docs 变更。重新生成 `references/source-map.md`，更新 `README.md` 中的快照 commit，并保留本地 Hono 工作流 guidance、evals 和 `code-spec` 父级路由入口。 |
...
| `react-query` | `https://github.com/TanStack/query/tree/main/docs/framework/react` | reference 模块：`.agents/skills/code-spec/references/react-query`；父级 guidance：`.agents/skills/code-spec/SKILL.md` data-fetching 与外部依赖相关章节 | 将本地 reference 模块复制到临时目录，在该临时副本中运行 `scripts/update-source-docs.ps1` 以刷新 React framework docs 快照，然后在同步有意义变更前，比较刷新后的临时模块与 `.agents/skills/code-spec/references/react-query`。保留本地 TanStack Query 工作流 guidance、evals 和 `code-spec` 父级路由入口。如果上游 guides、hooks、plugins 或文件名发生变化，更新 `references/doc-map.md`。 |
| `shadcn` | `https://github.com/shadcn/ui/tree/main/skills/shadcn` | reference 模块：`.agents/skills/code-spec/references/shadcn`；父级 guidance：`.agents/skills/code-spec/SKILL.md` shadcn/ui 相关章节 | 使用 sparse checkout 克隆 `https://github.com/shadcn/ui` 的 `skills/shadcn`，然后在审阅 diff 后将该子目录复制到 reference 模块中。保留并刷新嵌入在 `code-spec` 中的简明 shadcn/ui guidance，确保它直接、可读，并与当前 shadcn 规则一致。 |
...
| `vite-plus` | `https://github.com/voidzero-dev/vite-plus` | reference 模块：`.agents/skills/code-spec/references/vite-plus`；父级 guidance：`.agents/skills/code-spec/SKILL.md` Vite+ 章节 | 将仓库克隆到临时目录，然后使用 `skill-extractor` 从仓库文档重新生成 Vite+ reference 模块。不要只通过直接文件同步来更新；本地模块是提取并压缩后的结果。保留并刷新嵌入在 `code-spec` 中的简明 Vite+ guidance，确保它直接、可读，并链接到当前详细参考。 |
| `zod` | `https://github.com/colinhacks/zod/tree/main/packages/docs/content` | reference 模块：`.agents/skills/code-spec/references/zod`；父级 guidance：`.agents/skills/code-spec/SKILL.md` validation 与外部依赖相关章节 | 将本地 reference 模块复制到临时目录，在该临时副本中运行 `scripts/update-source-docs.ps1` 以刷新 Zod docs 快照，然后在同步有意义变更前，比较刷新后的临时模块与 `.agents/skills/code-spec/references/zod`。保留本地 Zod 工作流 guidance、evals 和 `code-spec` 父级路由入口。如果上游 docs routes、package docs 或文件名发生变化，更新 `references/doc-map.md`。 |

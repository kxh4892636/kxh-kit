...
完整读取 `<loop-x-skill-root-dir>/references/DOMAIN.md` 与 `<loop-x-skill-root-dir>/references/FLOW.md`，以其中的领域布局、Plan 生命周期和状态转移为权威约束。
...
3. **用户确认**：反复确认并迭代，直至通过；
4. **校验并交接**：从工作区根运行 `node <loop-x-skill-root-dir>/script/check-domain.mjs .`。
...
4. 每次暂停前同步 issue 状态、spec 派生视图与证据，并按 `<loop-x-skill-root-dir>/references/FLOW.md` 续租或释放；恢复时只信任文件与运行态，不依赖先前对话。

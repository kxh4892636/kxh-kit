...
const handleInit = (state, workspace, options, now) => {
	...
	return {
		changed: true,
		output: { ...stepOutput(PLAN_ROUTES[route][0]), plan: planKey },
	};
};
...
const handleClaimIssue = async (state, workspace, options, now) => {
	...
	if (issueState?.status === "active" && leaseIsActive(issueState.lease, now)) {
		if (issueState.lease.owner_session === session) {
			return {
				changed: false,
				output: { issue: issueId, ...stepOutput(nextStep(ISSUE_FLOW, issueState.cursor)), plan: planKey },
			};
		}
	...
	return {
		changed: true,
		output: { issue: issueId, ...stepOutput(next), plan: planKey },
	};
};
...
const handleBlockIssue = async (state, workspace, options, now) => {
	...
	await transitionIssue(
		path.resolve(workspace, planKey),
		issueId,
		"blocked",
		(content) => blockedBody(content, reason, releaseCondition),
	);
...
const dispatch = async (state, workspace, command, options, now) => {
	...
	if (
		["heartbeat-plan", "release-plan", "claim-plan", "heartbeat-issue", "release-issue"].includes(command)
	) {
...
export const executeFlow = async ({
	...
	return withStateTransaction(root, now, (state) =>
		dispatch(state, root, command, options, now()),
	);
};

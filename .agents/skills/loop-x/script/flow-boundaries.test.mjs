...
const readyPlan = async (workspace, session = "plan-session") => {
  await command(workspace, "init", { plan: PLAN_PATH, route: "main", session });
...
test.each([
  ...
  [
    "过短 lease",
    { plan: PLAN_PATH, route: "main", session: "s", "lease-seconds": "29" },
    "--lease-seconds",
  ],
  [
    "过长 lease",
    { plan: PLAN_PATH, route: "main", session: "s", "lease-seconds": "86401" },
    "--lease-seconds",
  ],
  [
    "非整数 lease",
    { plan: PLAN_PATH, route: "main", session: "s", "lease-seconds": "30.5" },
    "--lease-seconds",
  ],
  ["工作区外 plan", { plan: "../outside", route: "main", session: "s" }, "--plan 必须位于工作区内"],
...
test("init applies default and boundary leases, rejects duplicates, and exposes status", async () => {
  ...
  const initialized = await command(
    workspace,
    "init",
    { plan: PLAN_PATH, route: "main", session: "session", "lease-seconds": "30" },
    now,
  );
  assert.equal(initialized.session, "session");
  await assert.rejects(
    command(workspace, "init", { plan: PLAN_PATH, route: "main", session: "other" }, now),
    /已经初始化/,
  );
...
test("enter-plan validates initiators, entries, sessions, and completed main reuse", async () => {
  const workspace = await createWorkspace();
  await assert.rejects(
...
test("record-plan rejects wrong order, result, evidence, lease, and exhausted routes", async () => {
  const workspace = await createWorkspace();
  await command(workspace, "init", { plan: PLAN_PATH, route: "main", session: "owner" });
  await assert.rejects(
    recordPlan(workspace, "other", "to-issues", "started"),
    /资源由会话 owner 持有/,
  );
  await assert.rejects(recordPlan(workspace, "owner", "dev-gate", "ready"), /步骤顺序错误/);
  await assert.rejects(recordPlan(workspace, "owner", "to-issues", "completed"), /步骤顺序错误/);
...
test("rejects legacy schema states instead of migrating them", async () => {
  const workspace = await createWorkspace();
  await fs.mkdir(path.join(workspace, ".loop"), { recursive: true });
  await fs.writeFile(
    statePath(workspace),
...
test("rejects invalid active required-child state", async () => {
  const workspace = await createWorkspace();
  await command(workspace, "init", { plan: PLAN_PATH, route: "story", session: "owner" });
  await recordPlan(workspace, "owner", "to-story", "started");
  const state = JSON.parse(await fs.readFile(statePath(workspace), "utf8"));
  state.plans[PLAN_PATH].setup.active_step.phase = "invalid";
  await fs.writeFile(statePath(workspace), JSON.stringify(state));
  await assert.rejects(recordPlan(workspace, "owner", "grilling", "completed"), /phase 无效/);

  state.plans[PLAN_PATH].setup.active_step = {
    child_skill: "wrong",
    parent_cursor: 0,
    parent_skill: "to-story",
    phase: "child",
  };
  await fs.writeFile(statePath(workspace), JSON.stringify(state));
  await assert.rejects(
    recordPlan(workspace, "owner", "grilling", "completed"),
...
test("rejects legacy plans with omitted optional receipts and issues", async () => {
  const workspace = await createWorkspace();
  await fs.mkdir(path.join(workspace, ".loop"), { recursive: true });
  await fs.writeFile(
    statePath(workspace),

...
```powershell
node .agents/skills/loop-x/script/flow.mjs init --plan <plan-path> --route <main|story|issues> --session <session-id>
```

- `main`、`story`、`issues` 分别选择下文三条路径。`session-id` 必须在一个会话内稳定、在并行会话间唯一。
